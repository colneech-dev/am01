...
/* The wrapper's own comment on the ADDR_LCD_CMD/DATA/DATA8 write decode:
 * "Display writes are dropped if the shifter is still busy; the host polls
 * LCD_STAT before writing." gpio_ready is asserted regardless of whether the
 * write was actually accepted, so a write submitted while spi_busy is still
 * set completes the host-side handshake successfully and is silently lost.
 *
 * Nothing polled this before 2026-08-31. Isolated, well-spaced writes (the
 * init sequence's single-parameter commands, each followed by a ~150ms nap)
 * almost always survived anyway, because the SPI shifter (a handful of
 * microseconds at 6.25MHz) finishes long before the next syscall-paced write
 * arrives. CASET/PASET fire four DATA8 writes back-to-back with no delay
 * between them -- the one pattern most likely to race spi_busy -- and losing
 * even one of the four corrupts the whole address window, which is why the
 * panel showed backlight + a valid init but no image at all: RAMWR then had
 * nowhere valid to write. Confirmed on hardware 2026-08-31 via am01_probe
 * raw, which skips CASET/PASET entirely and writes pixels fine. */
static int wait_lcd_ready(am01_bus_t *bus)
{
