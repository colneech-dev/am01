...
if [ ! -f "$OUT/am01_qmtech_top.fp.json" ]; then
    ...
    mkdir -p "$OUT"
    SRL=0 FREQ=133.33 DEFINES=NO_XADC BRAM_OUTREG=1 BRAM_FP=1 BRAM_YBASE=40 \
        XDC="$XDC" \
...
echo "    DOA_REG=1 count: $(grep -ao '\"DOA_REG\": 1' "$OUT/am01_qmtech_top.fp.json" | wc -l) (expect 420)"

for s in $SEEDS; do
    d="seedrun/e2_s${s}"; mkdir -p "$d"
    echo "=== e2 seed $s -- $(date -Is) ==="
