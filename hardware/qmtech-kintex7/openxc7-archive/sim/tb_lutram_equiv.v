...
module tb;
    ...
    localparam THROUGHPUT = 2;   // must match the odo_gen throughput argument
    ...
    ref_2encrypt u_ref (clk, in,     read, out_ref, write_ref);
    tst_2encrypt u_tst (clk, in_tst, read, out_tst, write_tst);
    ...
    initial begin
        ...
        $display("  N=%0d brk=%0d blocks=%0d  (ref lutram=0, tst lutram=3, both latency 254)",
                 N, brk, blocks);
        t_start = $time;
