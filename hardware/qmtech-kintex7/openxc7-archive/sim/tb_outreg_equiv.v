...
module tb;
    ...
    // The negative control must perturb ONLY the core under test. Driving a
    // corrupted `in` into BOTH cores -- as this harness did until 2026-09-05 --
    // makes the control inert: both compute the same changed digests and the
    // queues still match, so +brk could never fail and the positive PASS
    // proved nothing. Same pattern as tb_lutram_equiv/tb_premix_equiv.
    wire [639:0] in_reg = (brk != 0 && nreg >= brk) ? (in ^ 640'd1) : in;

    ref_4encrypt u_ref (clk_h, in,     read, out_ref, write_ref);
    reg_4encrypt u_reg (clk_h, in_reg, read, out_reg, write_reg);
    ...
    initial begin
        ...
        for (i = 0; i < N; i = i + 1) begin
            ...
            if (i % 8 == 0) begin
                ...
                // Negative control lives on in_reg above, which feeds only the
                // core under test. Nothing to do to the shared `in` here.
                read = 1;
            end else begin
                read = 0;
            end
