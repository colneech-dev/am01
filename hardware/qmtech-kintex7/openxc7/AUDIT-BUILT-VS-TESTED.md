...
9. **Replicate `crypt.progress[1]`.** `encrypt.v:15330`, a 172-stage pipeline
   shift register whose bit 1 drives **644 sinks** and owns all three worst
   Vivado paths at 15.104 ns. `predictDelay` has no fanout term
   (`xilinx/arch.cc:831-862`) and HeAP divides weight by `users.size()`
   (`placer_heap.cc:812`), so the net that owns the failure is the one both
   models weight least. This is what Vivado's `phys_opt_design` does and is a
   plausible part of why it reaches 158.81 MHz on the same netlist.
   - `encrypt.v` is GENERATED by `odo_gen` — fix the generator or apply a
     tracked patch; do not hand-edit and lose it at the next regeneration.
   - **Caveat to hold:** 644 fanout is *Vivado's* bottleneck. nextpnr's own
     89.30 MHz run is limited by a different net (BRAM -> LUT,
     `round17.mid[1][158]`). The gain to OUR flow is unmeasured.
   - Estimated 63.55 -> ~100 MHz on the Vivado-routed path (INFERRED from the
     `vivado_path_prize.txt` K-table, not measured).
