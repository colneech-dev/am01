...
TAG="${TAG:-premix_$RTL_PINNED_COMMIT}"
...
if ! "$REPO/tools/odo_gen/odo_gen" "$EPOCH" 4 encrypt_4 --bram-out-reg --pipeline-premix \
        > gen/encrypt_premix.v; then
    echo "odo_gen FAILED"; exit 1
fi
[ -s gen/encrypt_premix.v ] || { echo "odo_gen produced nothing"; exit 1; }
...
_taps=$(grep -oE "get_key[0-9]+\(clk, period\[[0-9]+\]" gen/encrypt_premix.v \
...
if [ ! -f "$_netlist" ]; then
    ...
    for _src in gen/encrypt_premix.v "${RTL_SRCS[@]}"; do
...
if [ "$_need_synth" = "1" ]; then
    ...
    SRL=0 FREQ=133.33 DEFINES=NO_XADC BRAM_OUTREG=0 BRAM_FP=1 BRAM_YBASE=40 \
        ...
        "${RTL_SRCS[@]}" gen/encrypt_premix.v > "$OUT/build.log" 2>&1
...
for s in $SEEDS; do
    d="seedrun/premix_s${s}"; mkdir -p "$d"
