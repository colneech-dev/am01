...
foreach net $allnets {
    incr idx
    # Stride across the whole net list. Netlist order is highly correlated with
    # placement (the first thousands of nets are all intra-slice sbox registers),
    # so taking the first N nets samples ONLY manhattan-0 connections.
    ...
    foreach dly $dlys {
        if {$n >= $want} { break }
        # cap sinks per net so a few huge-fanout nets cannot dominate the sample
        if {$nsink >= $maxsink} { break }
