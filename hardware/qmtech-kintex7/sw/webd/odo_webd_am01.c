...
/* AM01 FORK, 2026-09-06. Diverges from sw/odo-webd/odo_webd.c in the
 * odo-miner-cyclonev tree ONLY in where the mutating handlers put their work.
 *
 * That tree targets a BusyBox-init image: /etc/odod.conf for the pool,
 * /etc/wpa_supplicant.conf for WiFi, and /etc/init.d/S90odod to restart the
 * miner. On the AM01 none of those exist -- it is systemd, the pool lives in
 * /etc/default/odo-miner installed from /boot/am01-miner.conf, and the WiFi in
 * /etc/wpa_supplicant/wpa_supplicant-wlan0.conf. The handlers wrote their
 * files, reported success, and nothing happened.
 *
 * Rather than restate those paths here and have two places to keep right, the
 * pool and WiFi handlers now drop REQUEST FILES for am01-panel-helper: the
 * same root-side helper the CYD panel uses, which already knows the paths,
 * validates every argument against an allow-list, and carries 51 tests. This
 * daemon runs as User=miner and /run/odod/request is miner-owned, so it needs
 * no privilege of its own to do it.
 *
 * Unchanged, because they were already right for this board: fan_boost and
 * reset_stats write the /run/odod files odo-miner polls, and reboot is
 * reboot. */
#define REQ_DIR     "/run/odod/request"
#define CONF_PATH   "/etc/odod.conf"        /* unused on AM01, see above */
#define WPA_CONF    "/etc/wpa_supplicant.conf"  /* unused on AM01 */
...
{
    /* NOT SUPPORTED ON THIS BOARD, and it says so rather than pretending.
     *
     * The version this was forked from wrote /etc/odo-ui.conf, which odo-ui
     * polled. The AM01's panel is the CYD: it stores dim level and timeout in
     * its OWN NVS, and the panel protocol (cyd_cmd.h) has no command to set
     * them -- FAN_BOOST, RESET_STATS, REBOOT, RESTART, SET_POOL, SET_WIFI,
     * WIFI_SCAN and PING, and nothing for the screen.
     *
     * So this wrote a file nothing reads, and the page reported "Screen
     * saved". Adding a protocol message is the real fix; until then a 501 is
     * the honest answer, and index.html no longer offers the control. */
    (void)body;
    {
        /* strlen, not a hand-counted literal: send_response takes a byte
         * count, and a count longer than the string sends whatever follows it
         * in memory. */
        static const char msg[] = "dim is set on the panel itself; this "
                                  "board has no remote screen control\n";
...
/* Drop a request for am01-panel-helper, exactly as cyd_panel.c does.
 *
 * The temp name carries a LEADING DOT on purpose: the path unit watches
 * /run/odod/request/<verb> and glob(3) does not match a leading dot, so a
 * half-written file cannot trigger the helper. Then rename(), which is atomic
 * within the directory. */
static int write_request(const char *verb, const char *body)
{
    char tmp[160], fin[160];
    snprintf(tmp, sizeof tmp, "%s/.%s.tmp", REQ_DIR, verb);
    snprintf(fin, sizeof fin, "%s/%s", REQ_DIR, verb);

    FILE *f = fopen(tmp, "w");
    if (!f)
        return -1;
    if (fputs(body, f) == EOF) { fclose(f); unlink(tmp); return -1; }
...
static void handle_config(int fd, const char *body)
{
    ...
    /* AM01: hand it to am01-panel-helper. Four lines, in the order
     * apply_set_pool() reads them -- host, port, worker, password. The helper
     * re-validates all four against its own allow-lists, writes
     * /boot/am01-miner.conf, re-runs the provisioning that installs it into
     * /etc/default/odo-miner, and restarts the miner.
     *
     * host2/port2 are accepted by the form and DROPPED here: this board's
     * odo-miner takes a single pool positionally and has no notion of a
     * fallback, so storing one would be a setting that silently does nothing.
     * testnet likewise. */
    char req[512];
    int rn = snprintf(req, sizeof req, "%s\n%s\n%s\n%s\n",
                      host, port, worker, pass[0] ? pass : "x");
    if (rn <= 0 || (size_t)rn >= sizeof req || write_request("set_pool", req) != 0) {
        {
            static const char msg[] = "could not queue the pool change\n";
...
static void handle_wifi(int fd, const char *body)
{
    ...
    /* AM01: hand it to am01-panel-helper -- two lines, ssid then psk, in the
     * order apply_set_wifi() reads them.
     *
     * The helper writes BOTH /etc/wpa_supplicant/wpa_supplicant-wlan0.conf and
     * the /boot copy, then restarts am01-wifi.service. Writing only /etc, as
     * the version this was forked from effectively did, gives a change that
     * works until the next boot and is then silently reverted by
     * am01-wifi-provision.
     *
     * An OPEN network is refused rather than half-supported: apply_set_wifi
     * requires a passphrase of 8..63 and writes key_mgmt=WPA-PSK
     * unconditionally, so an empty psk would be rejected there and the user
     * would see nothing happen. Better to say so here. */
    if (!psk[0]) {
        {
            static const char msg[] =
                "this board requires a WPA passphrase (8-63 chars)\n";
            send_response(fd, "400 Bad Request", "text/plain",
                          msg, sizeof msg - 1);
        }
        return;
    }
    {
        ...
        if (rn <= 0 || (size_t)rn >= sizeof req ||
            write_request("set_wifi", req) != 0) {
            {
                static const char msg[] = "could not queue the WiFi change\n";
                send_response(fd, "500 Internal Server Error", "text/plain",
                              msg, sizeof msg - 1);
            }
            return;
